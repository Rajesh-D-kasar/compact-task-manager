/**
 * CLI for interacting with demo contract
 */
...
const PRIVATE_STATE_ID = 'taskTrackerPrivateState';

const { network, config: networkConfig } = resolveNetwork();
const SEED = getOrCreateSeed(network);

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const zkConfigPath = path.resolve(__dirname, '..', 'contracts', 'managed', 'task-tracker');
...
const TaskTracker = await import(pathToFileURL(contractPath).href);

const compiledContract = CompiledContract.make('task-tracker', TaskTracker.Contract).pipe(
...
async function createProviders(walletCtx: WalletContext) {
  ...
  return {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'task-tracker-state',
      accountId,
...
async function main() {
  ...
  try {
    ...
    while (running) {
      console.log('─── Menu ───────────────────────────────────────────────────────');
      console.log('  1. View task status');
      ...
      switch (choice.trim()) {
        case '1': {
          console.log('\n  Reading task status from blockchain...');
          try {
        ...
        case '2': {
          // For demo purposes, we will pass a 32-byte array of 2s, 
          // which matches the mock commitment we created in deploy.ts
          console.log('\n  Submitting task completion transaction...');
          try {
            const private_criteria = new Uint8Array(32).fill(2);
            const tx = await deployed.callTx.complete_task(private_criteria);
